...
A->>A:onApplicationEvent
A->>A:onApplicationEnvironmentPreparedEvent
A->>A:initialize
A->>A:initializeSystem
A->>B:initialize
B->>B:initializeWithConventions
note right of B:获取Logback的标准配置文件(logback.xml)
B->>B:config=getSelfInitializationConfig()
opt config==null
note right of B:获取带spring文字的配置文件(logback-spring.xml)
...
participant C as GenericConfigurator
participant D as EventPlayer
participant E as ConversionRuleAction
A->>B:configureByResourceUrl
B->>B:new JoranConfigurator
B->>C:doConfigure
C->>C:各种前置动作。。。
C->>C:new SaxEventRecorder
C->>C:recorder.recordEvents
C->>C:doConfigure(final List<SaxEvent> eventList)
C->>C:buildInterpreter
C->>D:interpreter.getEventPlayer().play
D->>C:startElement
C->>C:callBeginAction
C->>E:begin
...
participant C as UnsynchronizedAppenderBase
participant D as OutputStreamAppender
participant E as LayoutWrappingEncoder
participant F as PatternLayout
A->>A:buildLoggingEventAndAppend
A->>A:callAppenders
A->>B:appendLoopOnAppenders
B->>C:doAppend
C->>D:append
D->>E:encode
E->>F:doLayout
